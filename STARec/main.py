...
def main(args):
    ...
    best_val_auc = 0
    min_lr = 1e-5
    scheduler = ReduceLROnPlateau(optimizer, "max", factor=0.9, patience=1000, verbose=True, min_lr=min_lr)
    model_folder = "./saved_models/"
    model_path = model_folder + args["exp_name"] + "_" + args["postfix"]
    if not os.path.exists(model_folder):
        os.makedirs(model_folder)

    dataset = SimpleDataset(user_item, mode='train', with_time=args['with_time'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--model", type=str,
                        choices=[
                            ...
                            'Motivate', 'ESMM', 'ESMM2', 'MMOE', 'DIN', 'DIEN', 'Transformer'
                        ],
                        default='LSTM', help="Model to use")
    parser.add_argument(
        '-d', '--dataset', type=str,
