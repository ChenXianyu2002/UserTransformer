            ...
            if label_where[t] != 0:
                temp_h.append(
                    torch.sum(torch.softmax(a1[:(t + 1), batch_sizes[t] -
                                            ...
                                            dim=0) * h[:(t + 1), batch_sizes[t] - label_where[t]:batch_sizes[t]],
                              dim=0))
                l1.append(torch.arange(batch_sizes[t] - label_where[t], batch_sizes[t]))
                l2.append(torch.clone(already_label[:batch_sizes[t]][-label_where[t]:]))
                already_label[:batch_sizes[t]][-label_where[t]:] += 1
        l1 = torch.cat(l1)
        l2 = torch.cat(l2)
        h = torch.zeros((batch_sizes[0], label_len, self.hidden_size),
...
class STAMP(nn.Module):
    ...
    def forward(self, inputs, label_len=4):
        ...
        for t in range(length):
            if label_where[t] != 0:
                temp_h.append(
                    torch.sum(self.w0(torch.sigmoid(x1[:(t + 1), batch_sizes[t] - label_where[t]:batch_sizes[t]] +
                                                    x2[t:(t + 1), batch_sizes[t] - label_where[t]:batch_sizes[t]] +
                                                    wms[t:(t + 1), batch_sizes[t] - label_where[t]:batch_sizes[t]])) *
                              x1[:(t + 1), batch_sizes[t] - label_where[t]:batch_sizes[t]], dim=0) +
...
class ESMM(nn.Module):
    ...
    def f1(self, inputs, label_len):
        ...
        already_label = torch.zeros(batch_sizes[0], dtype=torch.long, device=inputs[0].device)
        for t in range(length):
            if label_where[t] != 0:
                temp_h.append(torch.mean(inputs[:(t + 1), batch_sizes[t] - label_where[t]:batch_sizes[t]], dim=0))
                l1.append(torch.arange(batch_sizes[t] - label_where[t], batch_sizes[t]))
                l2.append(torch.clone(already_label[:batch_sizes[t]][-label_where[t]:]))
                already_label[:batch_sizes[t]][-label_where[t]:] += 1
        l1 = torch.cat(l1)
        l2 = torch.cat(l2)
        h = torch.zeros((batch_sizes[0], label_len, self.input_size), device=inputs[0].device)
        h[l1, l2] = torch.cat(temp_h)
        return h
...
class ESMM2(ESMM):
    def __init__(self, input_size, hidden_sizes, dropout_rate, batch_first=True):
        super(ESMM2, self).__init__(input_size, hidden_sizes, dropout_rate, batch_first)
...
class MMOE(ESMM):
    def __init__(self, input_size, hidden_sizes, dropout_rate, expert_num=3, batch_first=True):
        super(MMOE, self).__init__(input_size, hidden_sizes, dropout_rate, batch_first)
    ...
    def get_expert(self):
        expert_layer = nn.Sequential(self.dropout, nn.Linear(self.input_size, self.expert_sizes[0]), self.activate)
        for i in range(1, len(self.expert_sizes)):
            expert_layer.add_module('{}_{}_2'.format('expert', i), self.dropout)
            expert_layer.add_module('{}_{}_0'.format('expert', i),
                                    nn.Linear(self.expert_sizes[i - 1], self.expert_sizes[i]))
            expert_layer.add_module('{}_{}_1'.format('expert', i), self.activate)
        return expert_layer

    def get_gate(self):
        gate = nn.Sequential(nn.Linear(self.input_size, self.expert_num), self.softmax)
        return gate
    ...
    def forward(self, inputs, label_len):
        ...
        h_expert = torch.permute(h_expert, [1, 2, 0, 3])  # (bs,label_len,3,dim)
        h_ctr = self.mlp['ctr'](torch.sum(h_expert * torch.unsqueeze(self.gates['ctr'](inputs), dim=-1), dim=-2))
        h_cvr = self.mlp['cvr'](torch.sum(h_expert * torch.unsqueeze(self.gates['cvr'](inputs), dim=-1), dim=-2))
...
class FMLayer(nn.Module):
    def __init__(self, padding_idx=0, n=10, k=5, v=None):
        ...
        self.W1 = nn.Embedding(n, 1, padding_idx=padding_idx)  # 前两项线性层
        self.V = nn.Embedding(n, k, padding_idx=padding_idx) if v is None else v  # 交互矩阵
