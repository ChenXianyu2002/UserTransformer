...
class SearchPredictModel(tf.keras.layers.Layer):
    def __init__(self, all_features_num, input_dim, hidden_dim, predict_hidden_width,
                 ...
                 lr=1e-2, decay_step=1000, l2_reg=1e-4, multi=False):
        ...
        self.multi = multi
        self.fc_layer = tf.keras.layers.Dense(units=2 if multi else 1, use_bias=True, name='last')
    ...
    def optimize_loss(self, y, y_, training):
        y_ = tf.sigmoid(y_)
        base_loss = tf.reduce_mean(tf.losses.log_loss(labels=y, predictions=y_))
        loss = base_loss
        ...
        threshold = 0.5
        y, y_ = y[:, 0], y_[:, 0]
        ones_click, zeros_click = tf.ones_like(y), tf.zeros_like(y)
        eval_metric_ops = {
            "auc": tf.metrics.auc(y, y_),
            "acc": tf.metrics.accuracy(y, tf.where(y_ >= threshold, ones_click, zeros_click))
        }
        if not training:
    ...
    def optimize_loss_multi(self, y, y_, training):
        y_ = tf.sigmoid(y_)
        y_ = tf.stack((y_[:, 0], y_[:, 0] * y_[:, 1]), axis=1)
        base_loss_1 = tf.reduce_mean(tf.losses.log_loss(labels=y[:, 0], predictions=y_[:, 0]))
        base_loss_2 = tf.reduce_mean(tf.losses.log_loss(labels=y[:, 1], predictions=y_[:, 1]))
        loss = base_loss_1 + base_loss_2
        for v in tf.trainable_variables():
            loss += self.l2_reg * tf.nn.l2_loss(v)
        tf.summary.scalar('base_loss_1', base_loss_1)
        tf.summary.scalar('base_loss_2', base_loss_2)
        tf.summary.scalar('loss', loss)

        threshold = 0.5
        ones_click, zeros_click = tf.ones_like(y[:, 0]), tf.zeros_like(y[:, 1])
        eval_metric_ops = {
            "auc1": tf.metrics.auc(y[:, 0], y_[:, 0]),
            "acc1": tf.metrics.accuracy(y[:, 0], tf.where(y_[:, 0] >= threshold, ones_click, zeros_click)),
            "auc2": tf.metrics.auc(y[:, 1], y_[:, 1]),
            "acc2": tf.metrics.accuracy(y[:, 1], tf.where(y_[:, 1] >= threshold, ones_click, zeros_click))
        }
        if not training:
            return [base_loss_1, base_loss_2], loss, eval_metric_ops, y_, y
        gvs, v = zip(*self.optimizer.compute_gradients(loss))
        gvs, _ = tf.clip_by_global_norm(gvs, 5.0)
        gvs = zip(gvs, v)
        train_op = self.optimizer.apply_gradients(gvs, global_step=self.global_step)
        return [base_loss_1, base_loss_2], loss, eval_metric_ops, y_, y, train_op
    ...
    def get_rnn_input(self, x):
        ...
        x = tf.concat([x, x_label], -1) if self.use_label else x
    ...
    def call(self, inputs, training=True):
        ...
        if self.multi:
            y = tf.concat((tf.greater_equal(y, 1), tf.greater(y, 1)), axis=1)
        else:
            y = tf.greater(y, 1)
        y = tf.cast(y, tf.float32)
        optimize_func = self.optimize_loss_multi if self.multi else self.optimize_loss
        return optimize_func(y, y_, training)
